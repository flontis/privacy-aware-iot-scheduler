...
private static void spawnADU(int port){
    try (final CloseableHttpClient httpclient = HttpClients.createDefault()){
        final HttpPost httppost = new HttpPost("http://localhost:" + 20000 + "/adu/spawn");

        JSONObject json = new JSONObject();
        json.put("relatedSensorID", 1);
        json.put("relatedSensorAddress", "http://localhost:20000");
        json.put("algorithm", "birch");
        json.put("ID", 2);
        json.put("adumAddress", "http://localhost:" + port);
        json.put("anomalyReceiver", Arrays.asList("http://localhost:40000"));
...
//        spawnADU(30000);
        ...
        ResourceWebserverStarter communicationChannel = new ResourceWebserverStarter(new URI("ws://" + "130.61.143.123" +  ":4444"), uuid);
